...
function ModelInstanceManagementContent() {
  ...
  const fetchInstances = useCallback(async () => {
    try {
      setLoading(true);
      const result = await ModelInstanceService.getAllInstances();
  ...
  const handleViewGroupInstances = (groupId: number) => {
  ...
  const filteredInstances = filterGroupId
    ? instances.filter((i) => i.groupId === filterGroupId)
    : instances;
        ...
        <Space size="small">
          ...
          <Tooltip title="查看组实例">
            <Button
              ...
              size="small"
              icon={<EyeOutlined />}
              onClick={() => handleViewGroupInstances(record.groupId)}
            />
          </Tooltip>
          <Popconfirm
            title="确定要删除此实例吗？"
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          <Text type="secondary">管理所有模型实例的配置、状态和健康检查</Text>
        </div>
        <Space>
          <Select
            placeholder="按模型组筛选"
      ...
      <Card title={`实例列表 (${filteredInstances.length})`}>
        <Table
...
export default function ModelInstanceManagementPage() {
  return (
    <Suspense fallback={
      ...
      <ModelInstanceManagementContent />
    </Suspense>
  );
}
